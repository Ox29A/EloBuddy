            ...
            if (HealthbarEnabled || PercentEnabled)
                foreach (var unit in EntityManager.Heroes.Enemies.Where(u => u.IsValidTarget() && u.IsHPBarRendered))
                {
                    ...
                    if (HealthbarEnabled)
                    {
                        ...
                        var startPoint = new Vector2(unit.HPBarPosition.X + BarOffset.X + damagePercentage * BarWidth,
                            unit.HPBarPosition.Y + BarOffset.Y - 5);
                        var endPoint =
                            new Vector2(unit.HPBarPosition.X + BarOffset.X + currentHealthPercentage * BarWidth + 1,
                                unit.HPBarPosition.Y + BarOffset.Y - 5);
                    ...
                    if (PercentEnabled)
                        Drawing.DrawText(unit.HPBarPosition + PercentOffset, DrawingColor,
